...
vunit_common.add_source(lib, SIM_ROOT / "./dsm2_tb.sv")

# Create testbench
tb = lib.test_bench("dsm2_tb")

input_data_path = ""

def post_func(results):
    report = results.get_report()
    path = report.output_path / "modelsim" / "dsm2_output_data.csv"
    data = np.loadtxt(path, delimiter=',')
    vunit_common.generate_plot(data,
                               F_OUTPUT,
                               FIGURES / f'dsm2_fft_osr_{int(OSR)}.eps',
                               fft_only=True,
                               use_log_plot=True,
                               use_window=True,
                               show_plot=True,
...
def post_func_alt(results):
    ...
    ax.set_xlim([transition + diff, np.max(t)*1e6]) #transition - (diff/2.0), threshold + (diff*4.0)
    ax.set_ylim([0.7498, 0.7502]) #0.4, 0.8
...
def pre_config(output_path):
    path = WORKSPACE / "modelsim" / "dsm2_input_data.csv"
    #generate_sine(TEST_FREQ, TEST_SAMPLES, path, dither=0.0)
    #generate_step(1.0, TEST_SAMPLES, path, BIT_DEPTH)
    vunit_common.generate_dc(path, TEST_SAMPLES, BIT_DEPTH, 1.0)
    return True
